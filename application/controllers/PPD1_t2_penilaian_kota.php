...
class PPD1_t2_penilaian_kota extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd1/penilaian_tpitpu/penilaian_kota_t2.js?v=" . now("Asia/Jakarta");

                $data_page = array();
                $str = $this->load->view($this->view_dir . "index_kota_t2", $data_page, TRUE);
    ...
    function g_bahan()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT 	'2' kate,K.`id` 'mapwil', K.`id_kab`, K.`nama_kabupaten`,W.iduser, W.id kodewil, 'Tahap III' tahap
                FROM  `kabupaten` K 
                LEFT JOIN `tbl_user_t2_kabkot` W ON W.idkabkot = K.id
                WHERE W.iduser!='' AND K.urutan='1'
                GROUP BY K.id
                ORDER BY K.id_kab";
    ...
    function d_bahan()
    {
        ...
        //             LEFT JOIN `tbl_user` B ON A.iduser=B.id AND B.group=3 AND B.active_flag ='Y'
        //             LEFT JOIN `t_doc_tahap_penilaian_kabkota` C ON A.id = C.kbko AND C.isactive='Y'
        //             WHERE A.idkabkot=? AND B.group=3 AND B.active_flag ='Y' AND C.tahap='1'";
    ...
    function rekap_resume_tpitpu_by_user()
    {
        ...
        $this->excel->getActiveSheet()->setCellValue("D1", "NAMA KOTA");
        ...
        header("Content-Type:application/vnd.ms-excel");
        header("Content-Disposition:attachment;filename = rekap_resume_tpitpu_kota.xls");
        header("Cache-Control:max-age=0");
        $objWriter = PHPExcel_IOFactory::createWriter($this->excel, 'Excel5');
        $objWriter->save("php://output");
    }
