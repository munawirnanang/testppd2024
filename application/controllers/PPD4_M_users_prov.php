...
class PPD4_M_users_prov extends CI_Controller {
    var $view_dir   = "ppd4/PPD4_user/";
    var $js_init    = "main";
    var $js_path    = "assets/js/ppd4/PPD4_user/users.js";
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd4/PPD4_user/users_prov.js?v=".now("Asia/Jakarta");
        ...
        if($this->input->is_ajax_request()){
            try {
                ...
                $sql = "SELECT A.id, A.userid,A.`name`,A.email,A.`active_flag`,A.`group`,B.`groupid`,B.`name` groupname,A.satker, A.last_access, C.nama_provinsi
                        ...
                        WHERE B.`id`='4' AND A.satker = '$satkercode'";
     ...
     function add_act()
    {
        if($this->input->is_ajax_request()){
            try {
                ...
                if($list_data->num_rows() > 0){throw new Exception("User Id Sudah Ada",0);}
                
                $tambah="INSERT INTO tbl_user (userid,        `name`,                          `email`,      `password`,                      `group`,                          `satker`, `active_flag`,`cr_by`,`cr_dt`)
                   VALUES                     ('".$userid."', '".$this->input->post("name")."','".$email."', '".md5("pancasila"."monda")."','4','".$satker."','N','".$this->session->userdata(SESSION_LOGIN)->userid."','$current_date_time')";
