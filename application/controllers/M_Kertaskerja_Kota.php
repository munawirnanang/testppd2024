...
class M_Kertaskerja_Kota extends CI_Controller {
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/kertaskerja/kertaskerja_kota.js?v=".now("Asia/Jakarta");
            ...
            try {
                ...
                foreach ($list_data->result() as $v) {
                    ...
                    if (substr($v->attachments, -3) == 'rar') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".rar";
                    } elseif (substr($v->attachments, -3) == 'zip') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".zip";
                    } elseif (substr($v->attachments, -3) == 'pdf') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".pdf";
                    } elseif (substr($v->attachments, -3) == 'doc') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".doc";
                    } elseif (substr($v->attachments, -4) == 'docx') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".docx";
                    } elseif (substr($v->attachments, -4) == 'xlsx') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".xlsx";
                    } elseif (substr($v->attachments, -3) == 'xls') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".xls";
                    } elseif (substr($v->attachments, -3) == 'png') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".png";
                    } elseif (substr($v->attachments, -3) == 'PNG') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".png";
                    } elseif (substr($v->attachments, -3) == 'jpg') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".jpg";
                    } elseif (substr($v->attachments, -3) == 'JPG') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".jpg";
                    } elseif (substr($v->attachments, -4) == 'jpeg') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".jpeg";
                    } elseif (substr($v->attachments, -4) == 'jfif') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".jpg";
                    } elseif (substr($v->attachments, -4) == 'pptx') {
                        $rename = "kertas_kerja_" .str_replace(" ", "_", $v->name).".pptx";
                    } else {
