    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/management/users.js";
     ...
     function add_act()
    {
        if($this->input->is_ajax_request()){
            try {
                ...
                $_arr = array("4");
                if(in_array($groupid, $_arr)){
                    $this->form_validation->set_rules('satker','Id Provinsi','required|xss_clean');
                    if($this->form_validation->run() == FALSE){
                        throw new Exception(validation_errors("", ""),0);
                    }
                    $satker = decrypt_text($this->input->post("satker"));
                    ...
                    if($list_data->num_rows() == 0){throw new Exception("Data Provinsi tidak ada",0);}
                    $procode = $list_data->row()->id;
                }
                ...
                $tambah="INSERT INTO tbl_user (userid,        `name`,                          `email`,      `password`,                      `group`,                          `satker`, `active_flag`,`cr_by`,`cr_dt`)
                   VALUES                     ('".$userid."', '".$this->input->post("name")."','".$email."', '".md5("sundakelapa"."monda")."','".$this->input->post("group")."','".$satker."','Y','".$this->session->userdata(SESSION_LOGIN)->userid."','$current_date_time')";
