        ...
        if($this->input->is_ajax_request()){
            try {
                ...
                $tambah="INSERT INTO tbl_user (userid,        `name`,                          `email`,      `password`,                      `group`,                          `satker`, `active_flag`,`cr_by`,`cr_dt`)
                   VALUES                     ('".$userid."', '".$this->input->post("name")."','".$email."', '".md5("pancasila"."monda")."','7','".$satkercode."','Y','".$this->session->userdata(SESSION_LOGIN)->userid."','$current_date_time')";
                 ...
                 *select Id user Baru 
                 */
                $sql="SELECT T.* FROM tbl_user T WHERE T.userid = '".$userid."'";
                $list_data = $this->db->query($sql);
                if(!$list_data){
                    $msg = $session->userid." ".$this->router->fetch_class()." : ".$this->db->error()["message"];
                    log_message("error", $msg);
                ...
                $iduser     = $list_data->row()->id; //ID User
                
                /*
                 * tambah user daerah TPT
                 */
                $tambah_w ="INSERT INTO `tbl_user_daerah_tpt` (`iduser`,`idwilayah`,`cr_dt`)
                            SELECT  '".$iduser."' AS iduser, K.id, '$current_date_time' AS cr_dt 
                            FROM `kabupaten` K 
                            LEFT JOIN `provinsi` P ON P.id_kode = K.prov_id
                            WHERE P.id = '".$satkercode."' ";
                $status_save_w = $this->db->query($tambah_w);
                if(!$status_save_w){
                    throw new Exception($this->db->error()["code"].":Failed save data wilayah",0);
                }
