...
class PPD3_t3_penilaian extends CI_Controller {
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd3/PPD3_penilaian/PPD_3_penilaian_t3.js?v=".now("Asia/Jakarta");
            ...
            try {
                ...
                $userid= $this->session->userdata(SESSION_LOGIN)->id;
                    ...
                    $sql = " SELECT A.`id`,A.`judul`,A.`filename`
                            ...
                            WHERE A.`isactive`='Y' AND A.tahap ='2' AND B.`iduser`=? ";
                    $bind = array($userid);
            ...
            try {
                ...
                if($kate_wlyh=="PROV"){
                    ...
                    //get LIST tautan doc
                     $sql = " SELECT A.`id` mapid,A.`judul`,A.`tautan`
                            FROM `t_doc_tahap_penilaian_prov` A
                            JOIN `tbl_user_wilayah` B ON B.`id`=A.`mapid` 
                            WHERE A.`isactive`='Y' AND A.tahap ='2' AND B.`iduser`=? AND B.`idwilayah`=? ";
                ...
                elseif($kate_wlyh=="KAB" || $kate_wlyh=="KOTA"){
                    ...
                    $sql = " SELECT A.`id` mapid,A.`judul`,A.`tautan`
                            ...
                            WHERE A.`isactive`='Y' AND A.tahap ='2' AND B.`iduser`=? AND B.`idkabkot`=? ";
