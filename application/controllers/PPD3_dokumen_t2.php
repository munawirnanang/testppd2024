...
class PPD3_dokumen_t2 extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd3/PPD3_bahan_dukung/bahan_t2.js?v=" . now("Asia/Jakarta");

                $data_page = array();
                $str = $this->load->view($this->view_dir . "index_bahan_t2", $data_page, TRUE);
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                foreach ($list_data->result() as $v) {
                    ...
                    $str .= "<td class='text-center' style='padding: 3px; padding-left: 10px; padding-right: 10px; vertical-align: inherit; white-space: inherit; border: 1px solid black'><a href='$val_link' download='$rename' t]arget='_blank' class='btn text-primary'  title='Unduh Data'><i class='fas fa-download'></i><h7 class='mt-3 mb-0'><small></small></h7></a></td>";
                    $str .= "</tr>";
                }
                $response = array(
                    "status"    => 1,
    ...
    function d_bahan()
    {
        ...
        // Download
        $filename = "bahan_dukung_II_".$nama.".zip";
