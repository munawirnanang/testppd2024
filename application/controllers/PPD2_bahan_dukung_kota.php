 ...
 * date : 10 des 2020
*/
class PPD2_bahan_dukung_kota extends CI_Controller
{
    var $view_dir   = "ppd2/PPD2_bahan_dukung/";
    var $js_init    = "main";
    var $js_path    = "assets/js/admin/bahan_dukung/ppd2.js";
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $this->js_init    = "main";
                $this->js_tedit    = "main";
                $this->js_path    = "assets/js/ppd2/PPD2_bahan_dukung/bahan_dukung_kota.js?v=" . now("Asia/Jakarta");
    ...
    function g_wilayah()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //table wilayah
                $sql = "
                        SELECT 'kot' kate, K.`id` mapid,K.`id_kab` kode,K.`nama_kabupaten` nmprov
                        FROM `tbl_user_kabkot` A
                        JOIN `kabupaten` K ON K.`id`=A.`idkabkot`
                        WHERE A.`iduser`=? AND K.urutan ='1'";
