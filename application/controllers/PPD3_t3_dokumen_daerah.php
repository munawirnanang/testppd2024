...
class PPD3_t3_dokumen_daerah extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd3/PPD3_bahan_dukung/dokumen_daerah_t3.js?v=" . now("Asia/Jakarta");

                $data_page = array();
                $str = $this->load->view($this->view_dir . "index_th3", $data_page, TRUE);
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                if ($inp_katewlyh == "PROV") {
                    ...
                    //LIST PROVINSI
                    //                    $sql = "SELECT A.`id` mapid,P.id idprov, P.`id_kode` kode, P.`nama_provinsi` nmprov,P.`label`
					...
					SELECT A.`id`,A.provid, COUNT(1) jml  
					    ...
					    JOIN `t_doc_tahap_prov` C ON C.`docid`=A.`id` AND C.`tahap`=3
					    WHERE A.`isactive`='Y'
					    GROUP BY A.`id`
                 ...
                 */ elseif ($inp_katewlyh == 'KAB') {
                    ...
                    $sql = "SELECT A.`id` mapid,P.id idkab, P.`id_kab` kode, P.`nama_kabupaten` nmkab, IFNULL(JML.jml,0) jml_dok
                           ...
                           LEFT JOIN (
                                ...
                                JOIN `t_doc_tahap_kab` C ON C.`docid`=A.`id` AND C.`tahap`=3
                                WHERE A.`isactive`='Y' 
                                GROUP BY A.`id`
                 ...
                 */ elseif ($inp_katewlyh == 'KOTA') {
                    //LIST KOTA
                    $sql = "SELECT A.`id` mapid,P.id idkota, P.`id_kab` kode, P.`nama_kabupaten` nmkot, IFNULL(JML.jml,0) jml_dok
                            FROM tbl_user_t3_kabkot A
                    ...
                    if ($list_data->num_rows() == 0) {
                        ...
                        foreach ($list_data->result() as $v) {
                            ...
                            $tmp_kota = "class='btn btn-sm btn-primary waves-effect waves-light getDoc' data-id='" . $encrypted_kotaid . "'";
                            ...
                            $str .= "<div class='card' style='border: 2px solid rgba(38, 166, 154, 0.5); border-radius: 15px;'>";
                            $str .= "<div style='display: flex; align-items: center;'>";
                            $str .= "<img src='" . base_url() . "/assets/icons/PNG_KabupatenKota/" . $v->kode . "_" . $v->nmkot . ".png' alt='" . $v->nmkot . "' title='" . $v->nmkot . "' width='100' height='100' style='padding: 15px;'>";
                            $str .= "<div class='card-body' style='padding-top: 0.8rem; padding-left: 0px;'>";
                            $str .= "<div class='content-wilayah' style='display: flex; justify-content: space-between; align-items: center;'>";
                            $str .= "<h4 style='margin-bottom: 3px;'>" . $v->nmkot . "</h4>";
                            $str .= "</div>";
                            $str .= "<div class='mt-0'>";
                            $str .= "<p style='margin-bottom: 0px; color: #98a6ad !important; font-size: 0.8rem;'><strong>" . $v->jml_dok . "</strong> Dokumen </p>";
                            $str .= "</div>";
                            $str .= "</div>";
                            $str .= "<div class='btn-wilayah' style='float: right; margin-right: 3%;'>";
                            $str .= "<a href='javascript:void(0)' " . $tmp_kota . " style='border-radius: 0px; padding-left: 10px; padding-right: 10px;'>Unduh Dokumen Daerah <i class='fas fa-download' style='padding-left: 5px;'></i></a>";
                            $str .= "</div>";
                            $str .= "</div>";
                            $str .= "</div>";
                        }
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                foreach ($list_data->result() as $v) {
                    ...
                    $str .= "<td class='' style='padding: 3px; padding-left: 10px; padding-right: 10px; vertical-align: inherit; white-space: inherit; border: 1px solid black'><a href='" . $val_link . "' download='$rename' target='_blank' title='Klik untuk unduh dokumenn' >" . $v->judul . "</a></td>";
    ...
    function d_bahan()
    {
        ...
        if ($kate_wlyh == "PROV") {
            ...
            $filename = "bahan_daerah_III_" . $name . ".zip";
            /*
                     * check data PROV - end
                     */
