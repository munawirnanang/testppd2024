        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                if ($inp_katewlyh == "PROV") {
                    ...
                    $sql = "SELECT A.`id` mapid,P.id idprov, P.`id_kode` kode, P.`nama_provinsi` nmprov,P.`label`, IFNULL(JML.jml,0) jml_dok, COUNT(JML.jml) count_dok
                    ...
                    if ($list_data->num_rows() == 0) {
                        ...
                        foreach ($list_data->result() as $v) {
                            ...
                            $str .= "<p style='margin-bottom: 0px; color: #98a6ad !important; font-size: 0.8rem;'><strong>" . $v->count_dok . "</strong> Dokumen </p>";
                            $str .= "</div>";
                            $str .= "</div>";
                 ...
                 */ elseif ($inp_katewlyh == 'KAB') {
                    ...
                    //                           
                    //                            WHERE A.`iduser`=?";
                    $sql = "SELECT A.`id` mapid,P.id idkab, P.`id_kab` kode, P.`nama_kabupaten` nmkab, IFNULL(JML.jml,0) jml_dok, COUNT(JML.jml) count_dok
                            FROM tbl_user_t2_kabkot A
                            JOIN `kabupaten` P ON P.`id`=A.`idkabkot` AND P.`urutan`=0
                           LEFT JOIN (
