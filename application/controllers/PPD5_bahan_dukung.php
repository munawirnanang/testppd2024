...
class PPD5_bahan_dukung extends CI_Controller
{
    var $view_dir   = "ppd5/PPD5_bahan_dukung/";
    var $js_init    = "main";
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd5/PPD5_bahan_dukung/bahan_dukung_p.js?v=" . now("Asia/Jakarta");
    ...
    function g_bahan()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT K.`id`,K.`nama_kabupaten`,P.`id` provid,P.`nama_provinsi` FROM `kabupaten` K JOIN `provinsi` P ON P.id_kode= K.`prov_id` WHERE K.id=?";
                $bind = array($satker);
                $list_data = $this->db->query($sql, $bind);
                if ($list_data->num_rows() == 0)
                    throw new Exception("Data Kabupaten/Kota tidak ditemukan!", 0);
                $nmkabkot     = $list_data->row()->nama_kabupaten; //nama provinsi
                $provid     = $list_data->row()->provid; //nama provinsi
    ...
    function d_bahan()
    {
        ...
        // Download
        $filename = "Bahan_dukung_" . $nmkabkot . ".zip";
