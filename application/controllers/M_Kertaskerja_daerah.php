...
class M_Kertaskerja_daerah extends CI_Controller {
            ...
            try {
                ...
                foreach ($list_data->result() as $v) {
                    ...
                    if (substr($v->attachments, -3) == 'rar') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".rar";
                    } elseif (substr($v->attachments, -3) == 'zip') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".zip";
                    } elseif (substr($v->attachments, -3) == 'pdf') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".pdf";
                    } elseif (substr($v->attachments, -3) == 'doc') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".doc";
                    } elseif (substr($v->attachments, -4) == 'docx') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".docx";
                    } elseif (substr($v->attachments, -4) == 'xlsx') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".xlsx";
                    } elseif (substr($v->attachments, -3) == 'xls') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".xls";
                    } elseif (substr($v->attachments, -3) == 'png') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".png";
                    } elseif (substr($v->attachments, -3) == 'PNG') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".png";
                    } elseif (substr($v->attachments, -3) == 'jpg') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".jpg";
                    } elseif (substr($v->attachments, -3) == 'JPG') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".jpg";
                    } elseif (substr($v->attachments, -4) == 'jpeg') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".jpeg";
                    } elseif (substr($v->attachments, -4) == 'jfif') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".jpg";
                    } elseif (substr($v->attachments, -4) == 'pptx') {
                        $rename = "kertas_kerja_daerah_" .$v->name.".pptx";
                    } else {
                        $rename = $v->attachments;                   
                    }
                        ...
                        $str.="<td  class=''><a href='$downl' download='$rename' target='_blank' class='btn btn-xs btn-outline-info waves-purple waves-light '  title='Unduh Data'><i class='ion ion-md-archive'></i><h7 class='mt-3 mb-0'><small></small></h7></a></td>";
                        $str.="</tr>";
    ...
    function alldownload() {
        ...
        foreach ($list_data as $v) {
            ...
            if (!file_exists($downl)) {
                echo "File not found: $downl<br>";
                continue;
            }
    
            $ext = strtolower(pathinfo($v['attachments'], PATHINFO_EXTENSION));
            $rename = "kertas_kerja_tptd_" . $v['name'] . "_$counter." . $ext;
            $counter++;
    
            $file_content = read_file($downl);
            if ($file_content !== FALSE) {
                $this->zip->add_data($rename, $file_content);
            } else {
                echo "Failed to read file: $downl<br>";
            }
        }
    
        $this->zip->download('Kertas_kerja_tptd.zip');
    }
