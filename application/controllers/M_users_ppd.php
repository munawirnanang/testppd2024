...
class M_users_ppd extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/management/users_ppd.js?v=" . now("Asia/Jakarta");
                ...
                $str = $this->load->view($this->view_dir . "index_ppd", $data_page, TRUE);
    ...
    //                 $str = "<tr><td colspan='7'>Data tidak ditemukan</td></tr>";
    //             } else {
    ...
    //                     $str .= "<tr class='' title='list user admin'>";
    //                     $str .= "<td class='text-center' style='padding-left: 10px; padding-right: 10px; border: 1px solid black;'>" . $row->userid;
    //                     $str .= "<td class='text-center' style='padding-left: 10px; padding-right: 10px; border: 1px solid black;'>" . $row->name . "</td>";
    //                     $str .= "<td class='text-center' style='padding-left: 10px; padding-right: 10px; border: 1px solid black;'>" . $row->email . "</td>";
    //                     $str .= "<td class='text-center' style='padding-left: 10px; padding-right: 10px; border: 1px solid black;'>" . $row->groupname . "</td>";
    //                     $str .= "<td class='text-center' style='padding-left: 10px; padding-right: 10px; border: 1px solid black;'>" . ($row->active_flag == "Y" ? "<span class='badge badge-success'>Aktif</span>" : "<span class='badge badge-pink'>Tidak Aktif</span>") . "</td>";
    //                     $str .= "<td class='text-center' style='padding-left: 10px; padding-right: 10px; border: 1px solid black;'>" . $row->last_access . "</td>";
    //                     $str .= "<td class='text-center' style='padding-left: 10px; padding-right: 10px; border: 1px solid black;'><a href='javascript:void(0)'" . $tmp1 . "title='Edit Data'><i class='fas fa-pencil-alt'></i></a><a href='javascript:void(0)'" . $tmp2 . "title='Reset Password'><i class='fas mdi mdi-lock-reset'></i></a><a href='javascript:void(0)'" . $tmp3 . "title='Hapus Data'><i class='text fas fa-trash-alt'></i></a></td>";
    //                     $str .= "</tr>";
    //                 }
    //             }
    //             $json_data = array(
    //                 "data"  => $str,   // total data array
    //             );
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT A.id, A.userid,A.`name`,A.email,A.`active_flag`,A.`group`,B.`groupid`,B.`name` groupname,A.satker, A.last_access
                        FROM tbl_user A
                        INNER JOIN `tbl_user_group` B ON A.`group`=B.`id`
                        WHERE B.`id`='1' AND A.active_flag!='D'";
                ...
                foreach ($list_data->result() as $row) {
                    ...
                    $nestedData[] = "<a href='#'" . $tmp1 . "title='Edit Data'><i class='fas fa-pencil-alt'></i></a><a href='javascript:void(0)'" . $tmp2 . "title='Reset Password'><i class='fas mdi mdi-lock-reset'></i></a><a href='javascript:void(0)'" . $tmp3 . "title='Hapus Data'><i class='text fas fa-trash-alt'></i></a>";
                    $data[] = $nestedData;
                }
                $json_data = array(
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $tambah = "INSERT INTO tbl_user (userid,        `name`,                          `email`,      `password`,                      `group`,                          `satker`, `active_flag`,`cr_by`,`cr_dt`)
                   VALUES                     ('" . $userid . "', '" . $this->input->post("name") . "','" . $email . "', '" . md5("pancasila" . "monda") . "','1','','Y','" . $this->session->userdata(SESSION_LOGIN)->userid . "','$current_date_time')";
