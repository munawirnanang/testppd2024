...
class PPD3_modul3 extends CI_Controller
{
    ...
    //             //GET ASPEK MODUL 3
    //             $sql_aspek = "SELECT ASP.id idaspek,ASP.nama nmaspek FROM `r_mdl3_aspek` ASP ORDER BY `idaspek` ASC";
    //             $list_data_aspek = $this->db->query($sql_aspek);

    //             //GET INDIKATOR BY ASPEK MODUL 3
    ...
    //                             FROM r_mdl3_aspek ASP
    //                             JOIN r_mdl3_krtria KRT ON KRT.aspekid = ASP.id
    //                             JOIN `r_mdl3_indi` MI ON MI.krtriaid = KRT.id
    //                             JOIN `r_mdl3_sub_indi` SI ON SI.indiid = MI.id
    //                             JOIN `r_mdl3_item` I ON I.subindiid=SI.id
    //                             GROUP BY MI.nama
    ...
    //                 //LIST PROVINSI
    //                 $sql_nama_daerah = "SELECT A.`id` mapid,P.id idprov,P.id_kode,P.`nama_provinsi` nmprov,P.`label`,JML.jml, ST.id stts,IFNULL(RS.jml,0) jml_rsm
    //                         FROM tbl_user_t3_prov A
    //                         JOIN `provinsi` P ON P.`id`=A.`idwilayah`
    //                         LEFT JOIN(
    //                                 SELECT W.`idwilayah` idprov, COUNT(1) jml
    ...
    //                             //NILAI INDIKATOR MODUL 3
    //                             // sql untuk mengambil nilai indikator berdasarkan id indikator dan mapid
    //                             $sql_nilai_indi = "SELECT MI.nourut AS nourut, MI.id AS id_indikator, MI.nama AS nama, SUM(SKR.skor) AS total
    //                                         FROM `tbl_user_t3_prov` W
    //                                         JOIN `t_mdl3_skor_p` SKR ON SKR.`mapid`=W.`id`
    //                                         JOIN `r_mdl3_item` I ON I.`id`=SKR.`itemindi` 
    //                                         JOIN `r_mdl3_sub_indi` SI ON SI.`id`=I.`subindiid`
    //                                         JOIN `r_mdl3_indi` MI ON MI.`id`=SI.`indiid` AND MI.id=" . $vi->ID_INDIKATOR . "
    ...
    //                             $sql_item = "SELECT MI.id AS ID_INDIKATOR, MI.nama AS INDIKATOR, MI.bobot AS BOBOT, I.nama AS ITEM
    //                                         FROM r_mdl3_indi MI
    //                                         JOIN r_mdl3_sub_indi SI ON SI.indiid = MI.id
    //                                         JOIN r_mdl3_item I ON I.subindiid = SI.id
    ...
    //                             // $nilai = ($list_data_nilai_indi->result()[0]->total != null ? ($list_data_nilai_indi->result()[0]->total / $list_data_item->num_rows()) * $list_data_item->result()[0]->BOBOT : 0);
    //                             $nilai = ($list_data_nilai_indi->result()[0]->total != null ? ($list_data_nilai_indi->result()[0]->total / $list_data_item->num_rows()) : 0);
    ...
    //                 foreach ($list_data_nama_daerah->result() as $vd) {
    //                     if ($inp_katewlyh == "PROV") {
    //                         $sql_isi_aspek_kualitatif = "SELECT ASP.id idaspek,ASP.nama nmaspek,RSM.ksmplan kesimpulan, RSM.saran saran FROM `r_mdl3_aspek` ASP JOIN t_mdl3_resume_prov RSM ON RSM.`aspekid`=ASP.`id` AND RSM.`mapid`= " . $vd->mapid . " WHERE ASP.id = " . $va->idaspek . " ORDER BY ASP.id ASC ";
    //                     } elseif (($inp_katewlyh == "KAB") || ($inp_katewlyh == "KOTA")) {
    //                         $sql_isi_aspek_kualitatif = "SELECT ASP.id idaspek,ASP.nama nmaspek,RSM.ksmplan kesimpulan, RSM.saran saran FROM `r_mdl3_aspek` ASP JOIN t_mdl3_resume_kabkota RSM ON RSM.`aspekid`=ASP.`id` AND RSM.`mapid`= " . $vd->mapid . " WHERE ASP.id = " . $va->idaspek . " ORDER BY ASP.id ASC ";
    //                     }
    ...
    //                     $str_aspek .= "<p class='mb-0'><b>Indikator " . $no_indikator_card . "</b> &nbsp;: " . $via->INDIKATOR . "</p>";
    //                     $str_aspek .= "</div>";
    //                     $str_aspek .= "</div>";
    //                     $str_aspek .= "</div>";
    ...
    //                                 FROM `r_mdl3_item` IT
    //                                 JOIN `r_mdl3_sub_indi` SI ON SI.`id`=IT.`subindiid`
    //                                 WHERE SI.`indiid`=" . $via->ID_INDIKATOR . "
    //                                 ORDER BY SI.`nourut`,IT.`nourut` ASC";
    ...
    //                                 $str_aspek .= "<td colspan='7' class='text-uppercase'><b><small>Sub Indikator</small><br/>" . $v->nmsubindi . "</b></td>";
    //                                 $str_aspek .= "</tr>";
    //                             }
    //                             $idsubindi = $v->idsubindi;
    //                         }
    ...
    //                                 $sql_nilai_daerah = "SELECT IT.`nourut`, IT.`id`iditem,IT.`nama` nmitem,SI.`id` idsubindi ,SI.`nama` nmsubindi,SI.`istampil`,SP.`skor`, SP.sumber
    //                                                     FROM `r_mdl3_item` IT
    //                                                     JOIN `r_mdl3_sub_indi` SI ON SI.`id`=IT.`subindiid`
    //                                                     LEFT JOIN `t_mdl3_skor_p` SP ON SP.itemindi = IT.id AND SP.mapid=" . $vd->mapid . "
    //                                                     WHERE SI.`indiid`=" . $via->ID_INDIKATOR . " AND IT.id = " . $v->iditem . "
    //                                                     ORDER BY SI.`nourut`,IT.`nourut` ASC";
    //                             } elseif (($inp_katewlyh == "KAB") || ($inp_katewlyh == "KOTA")) {
    ...
    //                                                     FROM `r_mdl3_item` IT
    //                                                     JOIN `r_mdl3_sub_indi` SI ON SI.`id`=IT.`subindiid` AND SI.isprov='N'  
    //                                                     LEFT JOIN `t_mdl3_skor_k` SP ON SP.itemindi = IT.id AND SP.mapid=" . $vd->mapid . "
    //                                                     WHERE SI.`indiid`=" . $via->ID_INDIKATOR . " AND IT.id = " . $v->iditem . "
    //                                                     ORDER BY SI.`nourut`,IT.`nourut` ASC";
    //                             }
    ...
    function sttmnt_save()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $filename = "Dengan ini saya menyatakan bahwa penilaian ini dilakukan secara profesional, jujur, bertanggung-jawab, dan tidak atas dasar tekanan dari pihak manapun. Demikian pernyataan ini. Saya yang menyatakan:";
                date_default_timezone_set("Asia/Jakarta");
                $current_date_time = date("Y-m-d H:i:s");
