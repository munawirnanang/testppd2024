 ...
 * date : 10 des 2020
*/
class PPD8_M_Dokumen_Daerah_Kota extends CI_Controller
{
    var $view_dir   = "ppd8/";
    var $js_init    = "main";
    var $js_path    = "assets/js/ppd8/dokumen_daerah_kota.js";
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $this->js_init    = "main";
                $this->js_tedit    = "main";
                $this->js_path    = "assets/js/ppd8/dokumen_daerah_kota.js?v=" . now("Asia/Jakarta");
    ...
    function g_dokumen_all()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                $sql = "SELECT D.kabid, D.jenisid, D.judul, D.tautan, K.nama_kabupaten, D.cr_by, D.cr_dt
                        FROM `t_doc_kab` D  
                        JOIN `t_doc_kab_groupuser` G ON D.id = G.docid
                        JOIN kabupaten K ON D.kabid = K.id
                        WHERE D.isactive = 'Y' AND G.groupid = '8' AND K.nama_kabupaten LIKE 'Kota %'
                        ORDER BY D.kabid, D.jenisid ASC";
                ...
                $no = 1;
                $str = '';
                $link_d = base_url() . "attachments/kabkota/";
    ...
    function g_wilayah()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //table wilayah
                $sql = "SELECT 'kab' kate, K.`id` mapid,K.`id_kab` kode,K.`nama_kabupaten` nmprov
                        FROM t_doc_kab_groupuser KGU
                        JOIN `t_doc_kab` DK ON `KGU`.`docid`=DK.id
                        JOIN kabupaten K ON DK.kabid=K.id
                        WHERE KGU.groupid=8 AND K.`nama_kabupaten` LIKE 'KOTA %'
                        GROUP BY nmprov ASC";
        ...
        if ($this->input->is_ajax_request()) {
            try {
                ...
                if ($kate_wlyh == "prov") {
                    ...
                    $sql = "SELECT D.`id` mapid, D.judul, D.tautan
    ...
    function d_bahan_all()
    {
        ...
        $link_d = base_url() . "attachments/kabkota/";
        foreach ($list_data->result() as $list) {
            $filepath1 = FCPATH . 'attachments/kabkota/' . $list->tautan;
        ...
        $filename = "dokumen_daerah_kota.zip";
