...
class PPD1_t3_penilaian_kab extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/ppd1/penilaian_tpitpu/penilaian_kab_t3.js?v=" . now("Asia/Jakarta");

                $data_page = array();
                $str = $this->load->view($this->view_dir . "index_kab_t3", $data_page, TRUE);
    ...
    function detail_view()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //get jml item
                $sql = "SELECT I.`id` FROM `r_mdl3_item` I  LEFT JOIN `r_mdl3_sub_indi` SI ON SI.`indiid`=I.`id` AND SI.isprov='N'
    ...
    function Download_nilai()
    {
        ...
        $this->excel->getActiveSheet()->getStyle('I12:I120')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);
        $this->excel->getActiveSheet()->getStyle('L12:N120')->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);
