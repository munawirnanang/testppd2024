    ...
    function add_kk()
    {
        if ($this->input->is_ajax_request()) {
            try {
                ...
                //                $select_daerah="SELECT 	P.`id`, P.`id_kode`, P.`nama_provinsi`, P.`label`,P. `ppd`,W.iduser, 'Tahap III' tahap
                //                                FROM  `kabupaten` P 
                //                                LEFT JOIN `tbl_user_wilayah` W ON W.idwilayah = P.id
                //                                WHERE W.iduser='".$userid."'
                //                                    UNION
                //                                    SELECT 	P.`id`, P.`id_kode`, P.`nama_provinsi`, P.`label`,P. `ppd`,W.iduser, 'Tahap III' tahap
                //                                FROM  `kabupaten` K 
                //                                LEFT JOIN `tbl_user_t3_kabkot` W ON W.idkabkot = K.id
                //                                WHERE W.iduser='".$idusr."'
                //                         ";
