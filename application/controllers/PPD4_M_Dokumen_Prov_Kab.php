...
class PPD4_M_Dokumen_Prov_Kab extends CI_Controller {
    ...
    public function index(){
        if($this->input->is_ajax_request()){
            ...
            {                
                ...
                //common properties
                $this->js_init    = "main";
                $this->js_path    = "assets/js/admin/dokumen/dokumen_prov_kab.js?v=".now("Asia/Jakarta");
              
                $data_page = array(  
                );
                $str = $this->load->view($this->view_dir."index_prov_kab",$data_page,TRUE);
            ...
            try {
                ...
                if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
                        $sql.=" AND ( "
                                ...
                                . " OR K.`mapid` LIKE '%".$requestData['search']['value']."%' "
                                . ")";    
                }
    ...
    function update_dok()
    {
        if($this->input->is_ajax_request()){
            try {
                ...
                if(count($tmp)!=2)
                    throw new Exception("Invalid IDDDD");
                $kate_wlyh = $tmp[0];
                $idmap = $tmp[1];
