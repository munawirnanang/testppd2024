...
var main = function(){
    var datatable = function(){
        controller = "M_modul_2";
        
        var t_indi  = $("#t_indi");
        var t_sindi = $("#t_sindi");
        var t_item  = $("#t_item");
        var t_indiitem  = $("#t_indiitem");
        var t_judul = $("#t_ijudul");
        ...
        g_indi();
        $("#btnShwMdlKriteriaAdd").click(function(){
            $("#frmKriteriaAdd")[0].reset();
            $('#mdl_kriteria_add').modal('show');
        });
        ...
        $("#btnShwMdlJindiAdd").click(function(){
            $("#frmJitemAdd")[0].reset();
            $('#mdl_jitem_add').modal('show');
        });
        var frmAddJitem = $("#frmJitemAdd");
        frmAddJitem.validate({
                        errorElement: 'span',
            errorClass: 'error', 
            rules:{
                nama:{required:true},
...
//            },
            ...
            submitHandler: function(form) {
                var data1 = new FormData(frmAddJitem[0]);
                var url = controller+"/save_dinil";
                data1.append(csrf_name, $("#csrf").val());
                data1.append("id", $("#inp_indi").val());
                var data = data1;
        ...
        var frmEditJitem = $("#frmJitemEdit");
        frmEditJitem.validate({
            errorElement: 'span',
            errorClass: 'error', 
            rules:{
                id:{required:true},
            ...
            submitHandler: function(form) {
                var data1 = new FormData(frmEditJitem[0]);
                var url = controller+"/update_dinil";
                data1.append(csrf_name, $("#csrf").val());
                var data = data1;
                    ...
                    success:function(response){
                        ...
                        if(obj)
                        {
                            ...
                            if(obj.status === 1){
                                ...
                                g_sindi();
                                frmEditJitem[0].reset();
                                $('#mdl_jitem_edit').modal('hide');
                            }
                            else if(obj.status === 0){
                                sweetAlert("Perhatian!", obj.msg, "error");
                                loading.hide();
        ...
        t_judul.on('click', 'a.btnDel', function(e){
            e.preventDefault();
            var id      = $(this).data("id");
            var title   = $(this).data("title");
            Swal.fire({ 
                ...
                confirmButtonText: "Yes, hapus!" }).then((result) => {
                    if (result.value) {
                        ajax_url = controller+"/delete_dinil";
                        ajax_data = "id="+id;
                        ajax_data+="&"+csrf_name+"="+$("#csrf").val();
                        loading.show();
                        jQuery.ajax({
        ...
        t_judul.on("click","a.btnEdit",function(e){
            var _self = $(this);
            var id = _self.data("id");
            
            $('.has-error').removeClass('has-error');
            loading.show();
            ajax_url = controller+"/get_detail_dinil";
            ajax_data = "id="+id;
            ajax_data+="&"+csrf_name+"="+$("#csrf").val();
            jQuery.ajax({
                type: "POST",
                ...
                success:function(response){
                    ...
                    if(obj)
                    {
                        $("#csrf").val(obj.csrf_hash);
                        if(obj.status === 1){
                            $("#frmJitemEdit")[0].reset();
                            $('form#frmJitemEdit input[name="id"]').val(id);
                            $('form#frmJitemEdit input[name="nama"]').val(obj.data[0].nm);
                            //$('form#frmJitemEdit input[name="nourut"]').val(obj.data[0].nourut);
                            $('form#frmJitemEdit select[name="tampil"]').val(obj.data[0].istampil);
                            $('form#frmJitemEdit select[name="tag_prov"]').val(obj.data[0].isprov);
                            $('form#frmJitemEdit select[name="stts"]').val(obj.data[0].isactive);
                            //$('form#frmJitemEdit textarea[name="note"]').val(obj.data[0].note);
                            $('#mdl_jitem_edit').modal('show');
                            loading.hide();
                        }
                        else if(obj.status === 0){
                            loading.hide();
