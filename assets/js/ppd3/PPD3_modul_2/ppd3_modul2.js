var main = function(){
    controller = "PPD3_modul2";
    var datatable = function(){
        var twlyh = $("#t_wlyh");
        var tindi = $("#t_indi");
        var bnindi = $(".box-nilai-indi");
        var titem = $("#t_item");
                            ...
                            $.each( obj.list_data_aspek, function( key, value ) {
                                ...
                                $.each( obj.list_data_indi_by_aspek, function( key, valueIndi ) {
                                    if (value.nmaspek == valueIndi.aspek) {
                                        $.each( valueIndi.data, function (key, valueIndibyAspek) {
...
//                                window.location.href = base_url+"welcome";
        ...
        tindi.on("click","a.getDetail",function(e){
            ...
            var lblkriter   = _self.data("nmkriteria");
            var nourut   = _self.data("nourut");
            var bobot   = _self.data("bobot");
            var bobotindi   = _self.data("bobotindi");
            var skorindi   = _self.data("skorindi");
            $("#inp_indi").val(id);
            ...
            $(".lbl_hdr_nourut").html(nourut).parent("tr").show();
            $(".lbl_hdr_bobot").html(bobot).parent("tr").show();
            $(".lbl_hdr_bobotindi").html(bobotindi).parent("tr").show();
            
            $("#nilaiindi_1").val(skorindi);
            $("#idindi_1").val(id);
            $("#nomorindi_1").val(nourut);
            $("#namaindi_1").val(lblindi);

            $("#nilaiindi_2").val(skorindi);
            $("#idindi_2").val(id);
            $("#nomorindi_2").val(nourut);
            $("#namaindi_2").val(lblindi);
...
//                                window.location.href = base_url+"welcome";
        ...
        bnindi.on('change','._btnIsiIndi',function(){
            //if(clickable==true){
            var _self   = $(this);
            var id      = $('#idindi_1').val();
            var nomor   = $('#nomorindi_1').val();
            var nmindi  = $('#namaindi_1').val();
            var nilai   = $('#nilaiindi_1').val();
            // var id      = _self.data('id');
            ...
            if(nilai < 0 || nilai > 10  ){
                ...
                toastr["error"]("Skor harus lebih besar dari 0 kecil dari 10");
                // $(this).find("input").val("");
                var nilai   = $('#nilaiindi_1').val("");
                var nilai   = $('#nilaiindi_2').val("");
             return false;   
            }
            loading.show();            
            ajax_url = controller+"/save_score_indi";
            ajax_data="id="+id+"&nilai="+nilai+"&nomor="+nomor+"&nmindi="+nmindi;
            ajax_data+="&"+csrf_name+"="+$("#csrf").val();
            jQuery.ajax({
                    type: "POST",
                    url: base_url+ajax_url,
...
//                                _parent.children().children("._colSkor").html(_nilai);
                                ...
                                // $(".lbl_nilai").html(obj.nilai);
                                $('#nilaiindi_1').val(obj.ttl_skor);
                                $('#nilaiindi_2').val(obj.ttl_skor);
                                ...
                                $(this).find("input").val("");
                                $("._wrapper_infojudul").show();
                                g_item();
                                loading.hide();clickable=true;
                                sweetAlert("Data Gagal disimpan", obj.msg, "error");
...
//                                window.location.href = base_url+"welcome";
        ...
        bnindi.on('change','._btnIsiIndi2',function(){
            //if(clickable==true){
            var _self   = $(this);
            var id      = $('#idindi_2').val();
            var nomor   = $('#nomorindi_2').val();
            var nmindi  = $('#namaindi_2').val();
            var nilai   = $('#nilaiindi_2').val();
            // var id      = _self.data('id');
            // var nomor   = _self.data('nomor');
            // var nmitem   = _self.data('nmitem');
...
//                                window.location.href = base_url+"welcome";
        ...
        titem.on('change','._btnIsiIndiItem',function(){
            ...
            ajax_data="id="+id+"&nilai="+nilai+"&nomor="+nomor+"&nmitem="+nmitem;
            ajax_data+="&"+csrf_name+"="+$("#csrf").val();
            jQuery.ajax({
                    type: "POST",
                    url: base_url+ajax_url,
...
//                            $("#mdl_doc").modal("show");
                                ...
                                $('form#frmNilaiAdd input[name="nama"]').val("Penilaian Tahap 2 "+lblwlyh);
        ...
        frmNilaiAdd.validate({
            ...
            submitHandler: function(form) {
                ...
                jQuery.ajax({
                    xhr: function() {
                        ...
                        xhr.upload.addEventListener("progress", function(evt) {
                            if (evt.lengthComputable) {
                                ...
                                $(".progress-bar-upload-nilai").width(percentComplete + '%');
                                $(".progress-bar-upload-nilai").html(percentComplete+'%');
                            }
                        }, false);
                        return xhr;
                    },
                    ...
                    processData: false,
                     beforeSend: function(){
                        $(".progress-bar-upload-nilai").width('0%');
