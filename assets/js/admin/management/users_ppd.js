var main = function(){
    controller = "index.php/M_users_ppd";
    var datatable = function(){
        var table = $("#dataUser");
        
        // Setup - add a text input to each footer cell
        $('#dataUser tfoot th').each(function () {
            var title = $(this).text();
            $(this).html('<input type="text" style="border: 2px solid black;" placeholder="Search ' + title + '" />');
        });

        var datatable = $('#dataUser').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax":{
...
//                     { "width": "8px", "targets": 3}
 ...
 //                console.log(settings);
        ...
        $("#modal_add_kab").click(function(e){
             e.preventDefault();
                     var id      = $('form#form_edit input[name="iduser"]').val();
                     $("#inp_proid").val(id);
                     loading.show();
        ...
        $("#list_kab").on('click', '#save_popup', function(e){
                var dt = $('.radio:checked').data("data");
                alert(dt);
